...
Cacheman.prototype.run = function run(key, data, ttl, fn) {
  ...
  function run(i) {
    fns[i](key, data, ttl, function next(err, _data, _ttl) {
      // upon error, short-circuit
      if (err) return fn(err);

      // if no middleware left, summon callback
      if (!fns[i + 1]) return fn(null, _data, _ttl);
...
Cacheman.prototype.cache = function cache(key, data, ttl, fn) {
  ...
  cache.get(key, function get(err, res){
    ...
    if ('undefined' === typeof res) {
      return cache.run(key, data, ttl, function run(_err, _data, _ttl) {
        if (_err) return fn(_err);
        if ('undefined' !== typeof _data) data = _data;
        if ('undefined' !== typeof _ttl) ttl = _ttl;
